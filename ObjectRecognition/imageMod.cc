/* Title: imageMod.cc 
  ...
  Date: 9/20/2017
Note: Modified constructor and destructor to take care of pixel_labels matrix
*/ 
...
Image::Image(const Image &an_image){
  ...
  for(size_t i =0; i<num_rows(); i++) // set default labels to pixels
  {
     for(size_t j=0; j<num_columns(); j++)
	SetPixelLabel(i, j, -1); 
  }
...
Image::AllocateSpaceAndSetSize(size_t num_rows, size_t num_columns) {
  ...
  if(pixel_label != nullptr) DeallocateSpace(); 
  pixel_label = new int*[num_rows]; 
  for(size_t i=0; i<num_rows; ++i)
    pixel_label[i] = new int[num_columns]; 
...
Image::DeallocateSpace() {
  for (size_t i = 0; i < num_rows_; i++)
    delete pixels_[i];
  delete pixels_;
  for(size_t i=0; i< num_rows_; i++) // delete matrix pixel_label
    delete pixel_label[i]; 
  delete pixel_label; 
  pixels_ = nullptr;
  num_rows_ = 0;
  num_columns_ = 0;
